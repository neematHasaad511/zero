...
public class GameController {
    ...
    public boolean dfsRecursion(State state, int depth, int maxDepth, HashSet<State> visitedStates, ArrayList<State> path) {
        ...
        for (int i = 0; i < possibleMoves.size(); i++) {
            ...
            if (dfsRecursion(nextState, depth + 1, maxDepth, visitedStates, path)) {
                return true;
            }
        }
...
//                    nextState.printBoard();
    ...
    public void startGame() {
        ...
        while (true) {
            ...
            System.out.println("Enter direction (W, S, A, D) or 'dfsre' or 'bfs' or'dfs' or 'ucs' or  'exit' to quit:");
            String input = scanner.nextLine();

            if (input.equalsIgnoreCase("dfsRe")) {

                int maxDepth = 3;
                System.out.println("Starting DFS...");
                HashSet<State> visitedStates = new HashSet<>();
                ArrayList<State> path = new ArrayList<>();
                if (!dfsRecursion(getCurrentBoard(), 0, maxDepth, visitedStates, path)) {
